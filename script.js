...
const APIController = (function() {
  ...
  const _getFeaturedTracks = async (token) => {
      ...
      const result = await fetch(`https://api.spotify.com/v1/browse/categories/kpop/playlists/tracks?limit=${limit}`, {
          method: 'GET',
          headers: {
    ...
    const _getPopularTracks = async (token) => {
        ...
        const result = await fetch(`https://api.spotify.com/v1/playlists/37i9dQZEVXbKXQ4mDTEBXq`, {
            method: 'GET',
            headers: {
                'Content-Type' : 'application/json',
                'Accept' : 'applicaiton/json',
    ...
    const _getPlaylistByGenre = async (token) => {
        ...
        const result = await fetch(`https://api.spotify.com/v1/browse/categories/dinner/playlists?limit=${limit}`, {
            method: 'GET',
    ...
    const _getSearch = async (token, searchTerm) => {
        const result = await fetch(`https://api.spotify.com/v1/search?q=${searchTerm}&type=track&limit=10`, {
            method: 'GET',
            headers: {'Authorization' : 'Bearer ' + token}
        });
    ...
    const _getRecommendations = async (token, seedTracks) => {
        const result = await fetch(`https:api.spotify.com/v1/recommendations?limit=10&seed_tracks=${seedTracks}`, {
            method: 'GET',
            headers: {'Authorization' : 'Bearer ' + token}
        });
        const data = await result.json();
    ...
    const _getPlaylistsWithSong = async (token, seedTrack) => {
        const result = await fetch(`https://api.spotify.com/v1/search?q=${seedTrack}&type=playlist&limit=20`, {
            method: 'GET',
            headers: {'Authorization' : 'Bearer ' + token}
        });
        const data = await result.json();
    ...
    const _getNewTracks = async (token) => {
        ...
        const result = await fetch(`https://api.spotify.com/v1/playlists/37i9dQZF1DWSt89CX9de4L`, {
            method: 'GET',
            headers: {
                'Content-Type' : 'application/json',
                'Accept' : 'applicaiton/json',
    ...
    const _displayTracks = (songs) => {
        songs.forEach(song => {
            ...
            if (song != null) {
                ...
                const preview = `<input type="button" class="playbutton" onclick='UIController.playAudio("${song.preview_url}")' value="">`;
        ...
        songs.forEach(song => {
            ...
            if (track !== null) {
                ...
                if (track.preview_url == null) {
                    card.children[0].insertAdjacentHTML('beforeend', "<div><b>No preview available<b></div>");
                ...
                card.children[0].addEventListener("click", async (e) => {
                    ...
                    const recommendations = await APIController.getRecommendations(token, track.id);
                ...
                return { name: track.name, artist: track.artists[0].name, element: card }
                }
        });
    }
    ...
    const _displayRecommendations = (songs) => {
        songs.forEach(song => {
            ...
            if (song.preview_url == null) {
                card.insertAdjacentHTML('beforeend', "<p><b>No preview available<b><p>");
            }
            return { name: song.name, artist: song.artists[0].name, element: card }
        });
    }
