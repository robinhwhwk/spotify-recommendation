    ...
    searchtypes.forEach((searchtype) => {
      searchtype.addEventListener("click", async (e) => {
        ...
        if (searchtype.dataset.type == "artist") {
          searchtype.parentElement.previousElementSibling.textContent =
            "Artist";
        }
...
window.addEventListener("click", (e) => {
    ...
    if (lightbutton.classList.contains("light")) {
      ...
      if (songcards) {
        songcards.forEach((songcard) => {
          songcard.classList.toggle("light");
        });
      }
    } else {
  ...
  if (searchresults) {
    ...
    if (songcards && searchresults) {
      ...
      const generateform = document.getElementById("recommend_form");
      // if (generateform.contains(e.target) && numseeds > 0) {
      //   const seedArtists = seedlist.dataset.seedArtists.length > 1 ? seedlist.dataset.seedArtists : null;
      //   const seedTracks = seedlist.dataset.seedTracks.length > 1 ? seedlist.dataset.seedTracks : null;
      //   const seedGenres = seedlist.dataset.seedGenres.length > 1 ? seedlist.dataset.seedGenres : null;
      //   if (seedTracks && !seedArtists && !seedGenres) {
      //     generateform.action = `/core/recommendation/tracks/?tracks=${seedTracks}`
      //   } else if (seedArtists && !seedTracks && !seedGenres){
      //     generateform.action = `/core/recommendation/tracks/?artists=${seedArtists}`
      //   } else if (seedGenres && !seedTracks && !seedArtists){
      //     generateform.action = `/core/recommendation/tracks/?genres=${seedGenres}`
      //   } else if (seedTracks && seedArtists && !seedGenres) {
      //     generateform.action = `/core/recommendation/tracks/?tracks=${seedTracks}&artists=${seedArtists}`
      //   } else if (seedTracks && !seedArtists && seedGenres) {
      //     generateform.action = `/core/recommendation/tracks/?tracks=${seedTracks}&genres=${seedGenres}`
      //   } else if (!seedTracks && seedArtists && seedGenres ) {
      //     generateform.action = `/core/recommendation/tracks/?artists=${seedArtists}&genres=${seedGenres}`
      //   } else if (seedTracks && seedArtists && seedGenres) {
      //     generateform.action = `/core/recommendation/tracks/?tracks=${seedTracks}&artists=${seedArtists}&genres=${seedGenres}`
      //   }
        ...
        if (songcard.contains(e.target)) {
          ...
          if (numseeds < 5) {
            ...
            if (searchtype == "track") {
              ...
              clone.dataset.imageUrl = songcard.dataset.imageUrl;

              clone.textContent =
                clone.dataset.nameTrack + "-" + clone.dataset.nameArtist;
            ...
            if (searchtype == "artist") {
              ...
              xbutton.addEventListener("click", () => {
                removeSeed(
                  (trackId = null),
                  (artistId = songcard.dataset.artistId),
                  (genre = null)
                );
              });
    ...
    ytbuttons.forEach((ytbutton) => {
      if (ytbutton.contains(e.target)) {
        if (!ytbutton.dataset.videoId) {
          ...
          window.open(
            "https://www.youtube.com/watch?v=" + ytbutton.dataset.videoId
          );
        }
      }
    });
...
function removeSeed(trackId = null, artistId = null, genre = null) {
  if (trackId) {
    const seed = document.querySelector(
      `#search-keyword[data-track-id="${trackId}"]`
    );
    const xbutton = document.querySelector(
      `#seed-remover[data-track-id="${trackId}"]`
    );
    seed.remove();
    xbutton.remove();
  }
  if (artistId) {
    const seed = document.querySelector(
      `#search-keyword[data-artist-id="${artistId}"]`
    );
    const xbutton = document.querySelector(
      `#seed-remover[data-artist-id="${artistId}"]`
    );
    seed.remove();
    xbutton.remove();
  }
  if (genre) {
    const seed = document.querySelector(
      `#search-keyword[data-name-genre="${genre}"]`
    );
    const xbutton = document.querySelector(
      `#seed-remover[data-name-genre="${genre}"]`
    );
    seed.remove();
    xbutton.remove();
  }
